...
def load_last_pitcher_game():
    ...
    def load_last_pitcher_game(pitcher, season):
        for id_suffix in ['01', '02', '03', '04']:
            ...
            try:
                ...
                pitcher_gl = pitcher_gl[pitcher_gl['Rk'].apply(lambda x: pd.notna(x) and str(x).replace('.0', '').isdigit())].copy()
                ...
                last_game = pitcher_gl.loc[pitcher_gl['Rk'].astype(float).idxmax()].copy()
    ...
    # Process each pitcher with an 8-second delay between requests
