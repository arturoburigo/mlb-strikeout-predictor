...
def main(date=None, output_file=None):
    try:
        ...
        if not api_key:
            logger.warning("BETTINGPROS_API_KEY not found. Generating fake betting data instead.")
            generate_fake_betting_data(output_file, date_string)
            return
        ...
        print(f"[DEBUG] Saved betting data to: {output_file}")
        logger.info(f"Data saved to {output_file}")

    except Exception as e:
        logger.error(f"Error: {str(e)}", exc_info=True)

def generate_fake_betting_data(output_file, date_string):
    """
    Generate fake betting data when API key is not available
    """
    import pandas as pd
    import numpy as np
    ...
    # Generate 50 fake betting records
    n_records = 50
    
    # Create fake betting data
    data = {
        'Player': [f'Pitcher_{i}' for i in range(1, n_records + 1)],
        'Team': np.random.choice(['NYY', 'BOS', 'LAD', 'SFG', 'HOU', 'ATL', 'CHC', 'STL'], n_records),
        'Opponent': np.random.choice(['NYY', 'BOS', 'LAD', 'SFG', 'HOU', 'ATL', 'CHC', 'STL'], n_records),
        'Home': np.random.choice([0, 1], n_records),
        'Over Line': np.random.uniform(4.5, 8.5, n_records).round(1),
        'Over Odds': np.random.uniform(1.5, 2.5, n_records).round(2),
        'Under Line': np.random.uniform(4.5, 8.5, n_records).round(1),
        'Under Odds': np.random.uniform(1.5, 2.5, n_records).round(2),
        'API Projected Value': np.random.uniform(5.0, 7.0, n_records).round(1),
        'API Recommended Side': np.random.choice(['over', 'under'], n_records),
        'Streak': np.random.randint(1, 6, n_records),
        'Streak Type': np.random.choice(['over', 'under'], n_records),
        'Diff': np.random.uniform(-1.5, 1.5, n_records).round(2)
    }
    ...
    logger.info(f"Generated fake betting data with {n_records} records")
